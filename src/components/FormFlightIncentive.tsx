"use client";
import { FLIGHT_INCENTIVE_FIELDS, type FieldDef } from "@/lib/log-schemas";
...
export default function FormFlightIncentive({ values, onChange }: Props) {
  const sections = groupBySection(FLIGHT_INCENTIVE_FIELDS);
...
function FieldInput({
  ...
  return (
    <div className={`${colSpan}`}>
      ...
      {field.helper && (
        <div className="mt-0.5 text-[10px] text-[var(--ink-faint)]">{field.helper}</div>
      )}
    </div>
  );
}
